...
class DeliveriesLoader:
    ...
    def __init__(self, pg_dest: PgConnect, pg_saver: PgSaverD, logger: Logger):
    ...
    def run_copy(self, data):
        with self.pg_dest.connection() as conn:
            ...
            load_queue = data
            self.log.info(f"Found {len(load_queue)} documents to sync from GET/deliveries")
            if not load_queue:
            ...
            for d in load_queue:
                self.pg_saver.save_object(conn, str(d["order_id"]), d, update_ts)

            wf_settings.workflow_settings[self.LAST_LOADED_TS_KEY] = max(d["order_ts"] for d in load_queue)
